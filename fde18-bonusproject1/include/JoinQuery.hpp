...
/// Class which provides answers to queries on lineitem orders and customer
{
   ...
   ************************************************************************/
  std::string f_lineitem, f_order, f_customer;
  ...
  /**************************************************************************
   ...
   std::vector<std::string> getWordByIndex(std::string* s, std::set<int>* index);
   std::pair<uint64_t, uint64_t> computeAvg(char* partitionBegin, char* partitionEnd, char* fileEnd, std::unordered_set<int>* v_orders, unsigned index);
